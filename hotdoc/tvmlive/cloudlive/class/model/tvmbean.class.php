...
class Bean {
   ...
   private function get_edit(){
		...
		foreach ($this->data as $k=>$v){
			if(($this->data_ed[$k] != $v && $this->data_ed[$k]) || (isset($this->data_ed[$k]) && empty($this->data_ed[$k]))){
				$u[$k] = $this->data_ed[$k];
			}
		}
   	return $u;
	...
	public static function getList( $where , $value=null , $sort='' ,$limit=0 , $single=false,$gfields='',$group='',$having='' ){
    	$my = self::get_class_name();
    	if($limit>0){
            $_limit = " limit $limit";
    	}
	...
	if($sort){
        ...
        if($group){
            $_gorup = " group by ".$group;
    	}
         if($having){
            $_having = ' HAVING '.$having;
        }
	$sql = "select * $gfields from ".$my::TABLE.$_where.$_gorup.$_having.$_sort.$_limit;
    	if(is_array($value)){
    		$val = $value;
    	}else{
    		$val = array($value);
        ...
        public static function getTotal($where,$value,$group=''){
            if ($where) {
                $_where = " where " . $where;
            }
            if($group){
            ...
            $my = self::get_class_name();
            $sql = "select count(*) as total from ( select * from " . $my::TABLE . $_where . $_group .") as a";
