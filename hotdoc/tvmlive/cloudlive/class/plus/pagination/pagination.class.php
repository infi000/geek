...
class Pagination extends PaginalTable{
	...
	*/
	public function showGet(  $url = '' , $superfluous = '' ) {
		...
		//获取数字页码范围
                if($this->pages <= $this->showPages){
                    $showMinPage = 1;
                    $showMaxPage = $this->pages;
                }elseif($this->curr > $this->pages - floor( $this->showPages / 2 )){
                    $showMinPage = $this->pages - $this->showPages + 1;
                    $showMaxPage = $this->pages;
                }else{
                    $showMinPage = $this->curr - floor(($this->showPages-1) / 2);
                    if($showMinPage < 1){
                        $showMinPage = 1;
			$showMaxPage = $this->showPages;
                    }else{
                        $showMaxPage = $this->curr + floor($this->showPages / 2);
                        if($showMaxPage > $this->pages)
                            $showMaxPage = $this->pages;
                    }
                }
