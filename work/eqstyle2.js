...
//其中fullplayvideowin、smallzoomplayvideowin、playvideo这三个类进行了修改;
...
//		coursetitledivobj.innerHTML = '课程名称：' + activityobj["desc"];
		...
		window.playcourse = function() {
			...
			var urlstr = 'playcourse.htm?activityId='
					+ activityobj["activityid"] + '&stimestamp=' + stimestamp
					+ '&etimestamp=' + etimestamp + '&t='
					+ (new Date().getTime());
			...
			if (playprocessmainobj.length == 0) {
				var playmainhtml;
				if(!mainnew){
					playmainhtml = '<div id="playprocessmain" style="position:absolute;left:0px;top:70px;width:100%;height:100%;z-index:1000">'
						+ '<div id="playprocessvardiv" style="text-align:center;position:absolute;width:100%;height:70px;background-color:#dedede;left:0px;bottom:0px">'
					...
					playmainhtml = '<div id="playprocessmain" style="position:absolute;left:0px;top:'+top+'px;width:100%;height:100%;z-index:1000">'
					+ '<div id="playprocessvardiv" style="text-align:center;position:absolute;width:100%;height:'+top+'px;background-color:#FFFFFF;left:0px;bottom:0px">'
					+'<table style="width:100%;height:70%;margin-top:15%;"><tr><td><img class="imgbar" id="checkmuteimgbut" title="是否静音" onclick="checkMute(this)" src="./images/voicestart.png" data-mute="1"></td></tr>'
					+'<tr><td><input type="button" class="newimgbut" onclick="exitplaycourse(1)"  value="退 出" ></td></tr></table>'
					+ '</div>' + '</div>';
				}
...
window.smallzoomplayvodeowin = function() {
    ...
    $("#myConainerDivsmallzoomdiv").remove();
		...
		window.loadplayhtmlpage = function() {
			var url = './jwplayer/play.html?boxId=' + activityobj["boxid"]
					+ '&mpUser=' + activityobj["mpuser"] + '&activityId='
					+ activityobj["activityid"] + '&openid=' + loginopenid
					+ "&t=" + (new Date().getTime());
    ...
    loadplayhtmlpage();
		...
		window.beginflowwebsocket = function() {
			...
			if (activityobj["statusflag"] == "2") {
				url = 'flow.htm?boxId=' + activityobj["boxid"] + '&mpUser='
						+ activityobj["mpuser"] + '&activityId='
						+ activityobj["activityid"] + '&openid=' + loginopenid
						+ "&t=" + (new Date().getTime());
		...
		window.chancelmodifyimage1 = function(actionname, parm1, conainerobj,
			...
			if (actionname == "sendto" || actionname == "saveto"
					|| actionname == "savenote") {// 生成图片发送
				var notetype = actionname == "sendto" ? 2
						: actionname == "savenote" ? 3 : 1;
				...
				function animatefunc(base64data) {
					...
					if (notetype == 3) {
						...
						sendmsgbutobj = notetype == 1 ? $("#savenotemsgbut")
								: $("#sendmsgbut");
					}
					...
					newimg.src = base64data.indexOf("http://") != -1 ? base64data
		...
		window.modifyimage1 = function(conainerObj) {
			...
			if (imgabswidth == null || imgabsheight == null || dataUrl == null
					|| dataGuid == null) {
				alert("请等待图片加载完毕后重试!");
				return false;
			}

			var urlstr = './jSignature-master/examples/sign2.html?boxId='
					+ activityobj["boxid"] + '&mpUser=' + activityobj["mpuser"]
					+ '&activityId=' + activityobj["activityid"] + "&fakeId="
					+ loginfakeid + '&openid=' + loginopenid + "&t="
					+ (new Date().getTime());

			var iframehtml = '<iframe id="modifyimage1ifr" name="modifyimage1ifr" allowTransparency="true" src="'
					+ urlstr
					+ '" frameborder="0" style="position:absolute;left:0px;top:'
					+ imgheight
					+ 'px;width:100%;height:100%;z-index:8889"></iframe>'
			if (conainerObj == null) {
		...
		window.modifyimage = function() {
			...
			var heightval = (ua.match(/iPad/i) || ua.match(/android/i)) ? "height:"
					+ (coursedivobj.clientHeight) + "px"
					: "height:100%";

			var urlstr = './jSignature-master/examples/sign.html?boxId='
					+ activityobj["boxid"] + '&mpUser=' + activityobj["mpuser"]
					+ '&activityId=' + activityobj["activityid"] + "&fakeId="
					+ loginfakeid + '&openid=' + loginopenid + "&t="
					+ (new Date().getTime());

			var modifyuploadfiledivhtml = '<div id="modifyuploadfilediv" style="width:100%;'
					+ heightval
					+ ';position:absolute;left:0px;top:100%;z-index:1000">'
		...
		function loadNoteList(loadflag, callback) {
			...
			invokeMethod("eq.getNoteList", function call1(ref) {
				if (ref.status == "OK") {
					...
					var totalResults = parseInt(ref.totalResults
							|| (newstartIndex + 1));
		...
		function addnote(noteobj, notecontentlistdivobj) {
			...
			if (pptnodeobjslen > 0) {
				...
				notecontentlistdivobj.append('<a id="maodian_' + pptguid
						+ '" name="maodian_' + pptguid + '"></a>'
						+ note2html(noteobj, bgcolor2))
			}
		}
		function note2html(noteobj, beforebgcolor) {
			...
			if (notemime == "text") {
				...
				notecontentstr = '<img style="max-height:155px" onclick="showbigimgwithnote(\''
						+ noteid
						+ '\')" src="'
						+ coverurl
						+ '" data-url="'
			...
			var html = '<div data-background="'
					...
					+ '" id="note_'
					+ noteid
					+ '" style="width:230px;min-height:60px;border-bottom:1px solid #dedede;padding:8px;background-color:'
					+ beforebgcolor
					+ '">'
					+ '	<div style="line-height:23px;color:#0a397f;float:left" id="pptnode_'
					+ pptguid
					+ '" name="pptnode_'
					+ pptguid
					+ '">'
					+ notetypename
					+ '</div>'
					+ '	<div style="line-height:23px;color:#0a397f;float:right" id="pptnode_'
					+ cmdtimestamp
					+ '" name="pptnode_'
					+ cmdtimestamp
					+ '">'
					...
					+ '	<div class="notecontent" style="color:#505050;max-height:160px;overflow-x:hidden;overflow-y:auto;padding-left:6px">'
					+ notecontentstr
					+ '</div>'
					+ '	<div align="right" style="margin-top: 6px;">'
					+ '    <img onclick="notetoppt(\''
					+ pptguid
					+ '\','
					+ cmdtimestamp
					+ ')" title="转到PPT" alt="转到PPT" style="cursor:pointer;max-height:18px;margin-right:8px" src="./images/toppt1.png">'
					+ '    <img onclick="modifynote(\''
					+ noteid
					+ '\')" style="cursor:pointer;max-height:18px;margin-right:8px" src="./images/edit1.png">'
					+ '    <img onclick="deletenote(\''
					+ noteid
					+ '\')" style="cursor:pointer;max-height:18px;margin-right:8px" src="./images/newicon3.png">'
					+ '  </div>' + '<textarea style="display:none">'
					+ JSON.stringify(noteobj) + '</textarea>' + '</div>';
			return html;
		}
		window.showbigimgwithnote = function(noteid, showtype) {
			...
			var marginstr = '-' + (zoomhash.height / 2 + 28) + 'px 0px 0px -'
					+ (zoomhash.width / 2) + 'px';
			var html = '<div id="modifynotebg" style="z-index:8887;position:absolute;left:0px;top:0px;width:100%;height:100%;background-color:black;opacity:0.8;"></div>'
					+ '<div  id="modifynotewindiv" data-guid="'
					+ imgcontent.guid
					+ '" guid="'
					+ imgcontent.guid
					+ '"'
					+ ' style="background-image:url('
					+ notecontentobj.attr("src")
					+ ');background-size:100% 100%;'
					+ 'margin:'
					+ marginstr
					+ ';display:none;z-index:8887;position:absolute;left:50%;top:50%;width:'
					+ zoomhash.width
					+ 'px;height:'
					+ zoomhash.height
					+ 'px;overflow: hidden;">'
					...
					+ '" style="width:'
					+ zoomhash.width
					+ 'px;height:' + zoomhash.height + 'px;"></div>'
		...
		window.modifynote = function(noteid) {
			...
			$("body").append(animateobj);
			var html = '<div id="modifynotebg" style="z-index:8887;position:absolute;left:0px;top:0px;width:100%;height:100%;background-color:white;opacity:0.2;"></div>'
					+ '<div id="modifynotewindiv" align="center" style="box-shadow:0px 0px 10px #000000;display:none;z-index:8887;position:absolute;left:50%;top:50%;width:320px;height:460px;background-color:white;margin:-230px 0px 0px -210px">'
					+ '<div style="margin:8px;text-align:right" class="topbar"><img class="cancelbut" src="./images/close.png"></div>'
					+ '<div style="margin:8px;line-height:30px;color:#0067b6;font-size:18px;padding-left: 20px;text-align:left">修改课堂笔记</div>'
					+ '<textarea  style="box-shadow:inset 0px 1px 6px #9c9c9c;border-radius:6px;padding:3px;width:260px;height:280px;resize:none;margin:2px 10px 0px 10px;overflow:auto" id="modifynotecontent" onblur="return onbodyscroll()"></textarea>'
					+ '<div class="controls" align="center">'
					// +'<input type="button" class="imgbut savebut" value="保 存"
					// >'
					// +'<input type="button" class="imgbut cancelbut" value="取
					// 消" >'
					+ '<img src="./images/savebut2.png" class="savebut" style="margin:20px">'
					+ '</div>' + '</div>';
		...
		window.shownoteinfo = function() {
			...
			if (notelistdivobj == null) {
				...
				var divhtml = '<div id="notelistdiv" '
						+ 'style="display:none;background:#FFFFFF url() repeat-x left top'
						+ ';border-left:1px solid #cccccc;z-index:200;width:265px;height:'
						+ hv
						+ 'px'
						+ ';position:absolute;top:'
						+ tv
						+ 'px;right:-260px;">'
						+ '<div onclick="shownoteinfo(this)" style="cursor:pointer;font-weight:bold;background-color:#969696;color:#FFFFFF;height:86px;width:26px;margin-left:-26px;float:left;text-align:center;padding-top:8px">我<BR>的<BR>笔<BR>记</div>'
						+ '<div id="notecontentlistdiv" onscroll="loadmoredataWithScrollY(this,notecontentdivscroll)" style="-webkit-overflow-scrolling:touch;height:'
						+ hv
						+ 'px;overflow:hidden;overflow-y:auto"><div id="noteloadwaitdiv" style="margin:20px">加载中...</div></div>'
						+ '</div>';
				$("body").append(divhtml);
				loadNoteList(1);
		...
		window.XianChangHuDongFiles = function(isfirstload, packid, callback) {// 现场互动图片
			...
			if (activityobj["statusflag"] == "1"
				...
				appendval = false;
			} else if (activityobj["statusflag"] == "3"
					|| activityobj["statusflag"] == "4") {//
				reqparms["sort"] = "ASC";
		...
		window.getOnePPTInkStream = function(loadgifpicid, callbackshowimg) {
			...
			function showpptinkstream(filelist) {
				...
				var descItemobj = document.getElementById("desc_"
						+ loadgifpicid);
				if (descItemobj != null) {
					...
					if (descstr != "") {
						...
						contentbodydivobj
								.append('<div style="position:absolute;left:3px;bottom:2px;border:0px solid #595959;width:255px;background-color:white" id="itemdesc" readOnly >'
										...
										+ '<div style="max-height:150px;overflow-x:hidden;overflow-y:auto;text-align:left;padding:3px;">'
										+ descstr + '</div>' + '</div>');
					}
				}
			...
			if (loadgifpicidobj != null) {
				...
				var inkstreamobj = document.getElementById("inkstream_"
						+ loadgifpicid);
				...
				if (inkstreamobj == null) {
						...
						getcmd(
								...
								function(ref) {
									if (ref.status == "OK") {
										...
										var filelistlen = filelist == null ? 0
												: filelist.length;
										var jqobj1 = $(loadgifpicidobj);
										// statusflag :1进行中有直播 2:进行中无直播
										// 3:结束有回放视频 4:结束没有回放视频
										if ((activityobj["statusflag"] == "3" || activityobj["statusflag"] == "4")
												|| jqobj1.next().length > 0) {
                                            for(var i=0;i<filelistlen;i++){
                                                //替换imgurl地址，原来的cdn不能用了
											...
											var jsonstr = '<textarea id="inkstream_'
													+ loadgifpicid
													+ '" style="display:none">'
					...
					var filelist = JSON.parse(inkstreamobj.innerText
							|| inkstreamobj.value);
					showpptinkstream(filelist);
				}
			} else {
						...
						if (ref.status == "OK") {
							...
							for ( var i = 0; i < filelistlen; i++) {
								...
								if (contenttype == "image") {
									 ...
									 * 
									 */
									filehtml += '<a class="zlfile" data-url="'
											+ imgurl
											+ '" data-load="0">'
											// +'<div
											...
											+ '<img src="'
											+ coverurl
											+ '" data-img="1" class="img" onclick="viewpackonefile(this,1)">'
											+ '<div class="title">'
											+ fileobj.title
											+ '</div>'
											// +'</div>'
											+ (zuxiangviewtype == 2 ? ''
													: '<img  onclick="viewpackonefile(this,2)" src="./images/down.png" class="down">')
							...
							if (type1 == 1) {// 下载文件 &&packjsonobj.packmime!="VIDEO"
								if(filehtml){
									...
									var cssstr = height1val < 500 ? 'height:100%;'
											: '';
									zlpacklistdivobj
											.append('<div id="packfilessdiv" style="'
													+ cssstr
													+ 'min-height:500px;background-color:#212121;width:300px;position:absolute;top:0px;left:300px">'
													+ filehtml + '</div>');
									...
									$("#packfilessdiv")
											.animate(
													...
													function() {
														...
														$("#packtitlediv")
																.html(
																		'<a href="javascript:cancel2pack()" style="color:white;font-size:12pt;"><img src="./images/back.png" align="center">&nbsp;&nbsp;'
																				+ packjsonobj.title
																				+ '</a>');
													});
								}else{
