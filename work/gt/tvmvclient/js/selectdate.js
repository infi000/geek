...
//	var height=175;
	...
	//计算Div在窗口中的位置
	if(e.x+document.documentElement.scrollLeft+width > document.body.clientWidth){
		style.left=(e.pageX?e.pageX:(document.body.clientWidth-width-10))+"px";
	}else{
		style.left=(e.pageX?e.pageX:e.x+document.documentElement.scrollLeft)+"px";
	}
	style.top=((e.pageY?e.pageY:e.y+document.documentElement.scrollTop)+10)+"px";
...
function showPicker() {
	...
	//display year
	html+="<font style=\"cursor:pointer\" onmousedown=\"incYear(-1)\" color=white><img src=\"images/calender/back.gif\" /></font>&nbsp";
	html+="<font color=" + (isThisYear ? "green":"#ED1C24") + ">" + displayYear + "年</font>";
	html+="&nbsp<font style='cursor:pointer' onmousedown=\"incYear(1)\" color=white><img src=\"images/calender/next.gif\" /></font>&nbsp;&nbsp;&nbsp;";
	//display month
	html+="<font style=\"cursor:pointer\" onmousedown=\"incMonth(-1)\" color=white><img src=\"images/calender/back.gif\" /></font>&nbsp";
	if(displayMonth<10)html+="&nbsp";
	html+="<font color=" + (isThisMonth ? "green":"#ED1C24") + ">" + months[displayMonth] + "</font>";
	if(displayMonth<10)html+="&nbsp";
	html+="&nbsp<font style=\"cursor:pointer\" onmousedown=\"incMonth(1)\" color=white><img src=\"images/calender/next.gif\" /></font>";
	//目标输入框为只读时显示清除按钮
	if(formElement.readOnly) {
		//判断输入框是否为只读，如果是的话，显示清除按钮
		html+="&nbsp;<span onclick='formElement.value=\"\";hidePicker();' style=\"cursor:pointer;\"><img src=\"images/calender/btn_clear.gif\" alt=\"清除\"/></span>";	
	}
	if(tformat > timeswitch) {
		html+="&nbsp;<span onclick='hidePicker();' style=\"cursor:pointer;\"><img src=\"images/calender/btn_close1.gif\" /></span>";	
	}
	html+="</td></tr>";
	//display day
	...
	for (var intWeek = 0; intWeek < 6; intWeek++) {
		...
		if(intWeek+1==6 && tformat > timeswitch) {
			...
			html+="<td colspan='7' borderColor=#ffffff><div style='margin:-1.5px;'><input maxlength='2' onmousedown='this.select()' name='hour' id='hour' style='border: 1px solid #CCCCCC;font-size:12px;font-family:宋体;line-height:12px;height:20px;width:20px;text-align:center;padding:2px;' size='1' value='"+ (hour<10?'0'+hour:hour) +"' type='text'> 时 <input maxlength='2' onmousedown='this.select()' value='"+ (minutes<10?'0'+minutes:minutes) +"' style='border: 1px solid #CCCCCC;font-size:12px;font-family:宋体;line-height:12px;height:20px;text-align:center;width:20px;padding:2px;' name='minutes' size='1' type='text'> 分 <input maxlength='2' onmousedown='this.select()' name='second' style='border: 1px solid #CCCCCC;font-size:12px;line-height:12px;height:20px;text-align:center;width:20px;padding:2px;' value='"+ (second<10?'0'+second:second) +"' size='1' type='text'> 秒</div></td>";
		}
		html+="</tr>";
